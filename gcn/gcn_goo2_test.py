...
for epoch in range(FLAGS.epochs):
    ...
    feed_dict = OO_construct_feed_dict(features, support, y, train_mask_0, placeholders, alpha, beta, f_u)
    ...
    # Validation
    cost, duration = evaluate(features, support, y, val_mask_0, placeholders, alpha, beta, f_u)
    cost_val.append(cost)

    if epoch > FLAGS.early_stopping and cost_val[-1] > np.mean(cost_val[-(FLAGS.early_stopping+1):-1]):
        break

_, test_Z, _ = evaluate_with_predictions(features, support, y, test_mask, placeholders, alpha, beta, f_u)
