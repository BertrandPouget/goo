...
# Cross_validation
err_array = array(0, dim = c(n_zetas, n_kappas, n_thetas))
pb = progress_bar$new(total = n_kappas * n_thetas * n_zetas * n_fold)

for (i_zeta in 1:n_zetas)
{
  zeta = zetas[i_zeta]
  d = e^zeta * c^(1-zeta)
  for (i_kappa in 1:n_kappas)
  {
    ...
    for (i_theta in 1:n_thetas)
    {
      ...
      for (n in 1:n_fold)
      {
        f_u = compute_f_u(d,data$osm_surf,train_ids_CV[[n]],kappa)
        f_p = 1 - f_u
      ...
      err = mean(err_val_n)
      err_array[i_zeta,i_kappa,i_theta] = err
    }
  }
}

best_combo = arrayInd(which.min(err_array),c(n_zetas,n_kappas,n_thetas))
best_zeta = zetas[best_combo[1]]
best_kappa = kappas[best_combo[2]]
best_theta_1 = thetas[best_combo[3],1]
best_theta_2 = thetas[best_combo[3],2]

d = e^best_zeta * c^(1-best_zeta)

f_u = compute_f_u(d,data$osm_surf,train_ids,best_kappa)
f_p = 1 - f_u
